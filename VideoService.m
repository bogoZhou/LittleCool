...
+ (UIImage*) thumbnailImageForVideo:(NSURL *)videoURL atTime:(NSTimeInterval)time {
    ...
    {
        UIImage *thumbImage = [[[UIImage alloc] initWithCGImage:thumbnailImageRef] autorelease];
...
//    }
    ...
    [avAssetExportSession exportAsynchronouslyWithCompletionHandler:^(void){
        ...
        BOOL blHave=[[NSFileManager defaultManager] fileExistsAtPath:avAssetExportSession.outputURL.path];
        if (!blHave) {
            NSLog(@"不存在- > %@",avAssetExportSession.outputURL.path);
            //        return ;
        }else {
            NSLog(@"存在- > %@",avAssetExportSession.outputURL.path);
        }
