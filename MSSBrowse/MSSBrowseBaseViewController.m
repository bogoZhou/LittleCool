...
@interface MSSBrowseBaseViewController ()<TCCopyableLabelDelegate>
...
@property (nonatomic,strong) TCCopyableLabel *contentLabel;
...
- (void)createBrowseView
{
    ...
    _countLabel.frame = CGRectMake(0, 10, _screenWidth, 50);
    ...
    //创建顶部详情图层
    MSSBrowseModel *model = _browseItemArray[_currentIndex];
    
    CGSize titleSize = [model.imageContent boundingRectWithSize:CGSizeMake(kScreenSize.width - 60, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size;
    
    _contentView = [[UIView alloc] initWithFrame:CGRectMake(0, kScreenSize.height - 58 - titleSize.height - 30, kScreenSize.width, titleSize.height + 30)];
    _contentView.backgroundColor = [kBlackColor colorWithAlphaComponent:0.3];
    
    [_bgView addSubview:_contentView];
    
    _contentLabel = [[TCCopyableLabel alloc] initWithFrame:CGRectMake(30, 15, _contentView.sizeWidth - 60, _contentView.sizeHeight - 30)];
    _contentLabel.delegate = self;
    [_contentLabel setCopyingEnabled:YES];
    _contentLabel.text = model.imageContent;
    ...
    for (NSInteger i = 0; i < 2; i ++) {
        UIView *deepView = [[UIView alloc] initWithFrame:CGRectMake(kScreenSize.width/2 * i, _bgView.sizeHeight - 58, kScreenSize.width/2, 58)];
        deepView.backgroundColor = kClearColor;
        [_bgView addSubview:deepView];
        
        UIView * subView = [[UIView alloc] initWithFrame:CGRectMake((deepView.sizeWidth - 80)/2, 0, 80, deepView.sizeHeight)];
        subView.backgroundColor = kClearColor;
//        subView.center = deepView.center;
        [deepView addSubview:subView];
        
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake((subView.sizeWidth - 25)/2, 10, 25, 25)];
        if (i == 0) {
            imageView.image = [UIImage imageNamed:@"save"];
        }else{
            imageView.image = [UIImage imageNamed:@"share"];
        }
        [subView addSubview:imageView];
        
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, imageView.allHeight, subView.sizeWidth, 20)];
        if (i == 0) {
            nameLabel.text = @"保存";
        }else{
            nameLabel.text = @"分享";
        ...
        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, subView.sizeWidth, subView.sizeHeight)];
        
        if (i == 0) {
            [button addTarget:self action:@selector(saveImageButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        }else{
            [button addTarget:self action:@selector(shareImageButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        }
...
- (void)saveImageButtonClick:(UIButton *)button{
    ...
    [imageView sd_setImageWithURL:[NSURL URLWithString:model.bigImageUrl] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
- (void)shareImageButtonClick:(UIButton *)button{
    MSSBrowseModel *model = _browseItemArray[_currentIndex];
    NSArray *array = @[model.bigImageUrl];
    [BGShareHelper shareWeChatByController:self imageArray:array];
}
